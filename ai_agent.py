        ...
        async for message in ws:
            try:
                ...
                if data["type"] == "newToken":
                    mint = data["data"]["mint"]
                    TOKEN_CREATED_AT[mint] = data["data"]["timestamp"] / 1000

                elif data["type"] == "tokenTrade":
                    trade = data["data"]
                    mint = trade["token"]
                    ...
                    # Only track early trades (<30 sec)
                    ...
                    if ts - created <= 30:
                        ...
                        # Score wallet in real-time
                        profile = score_wallet(wallet)
                        if profile["is_elite"] and wallet not in ELITE_WALLETS:
                            ELITE_WALLETS.add(wallet)
                            print(f"✅ ELITE WALLET PROMOTED: {wallet} | Win Rate: {profile['win_rate']}, Avg ROI: {profile['avg_roi']}x")
